...
const XLSXUtils = require('xlsx-js-style');
...
const XLSXUtils__namespace = /*#__PURE__*/_interopNamespaceCompat(XLSXUtils);
const XLSXUtils__default = /*#__PURE__*/_interopDefaultCompat(XLSXUtils);
...
function getWorksheetColumnIds(worksheet) {
  const columnRange = XLSXUtils__namespace.utils.decode_range(worksheet["!ref"] ?? "");
  const columnIds = [];
  for (let C = columnRange.s.c; C <= columnRange.e.c; C++) {
    const address = XLSXUtils__namespace.utils.encode_col(C);
...
function applyGroupBorders(worksheet, params) {
  const start = XLSXUtils__namespace.utils.decode_cell(params.start);
  const end = XLSXUtils__namespace.utils.decode_cell(params.end);
  for (let r = start.r; r <= end.r; r++) {
    for (let c = start.c; c <= end.c; c++) {
      const cellRef = XLSXUtils__namespace.utils.encode_cell({ c, r });
      const cell = worksheet[cellRef] || { t: "z" };
      cell.s = deepmergeTs.deepmerge(cell.s ?? {}, {
        border: {
          ...cell.s?.border ?? {}
        }
      });
...
function getSheetRange(params) {
  return XLSXUtils__namespace.utils.encode_range({ s: { c: 0, r: 0 }, e: { c: params.width - 1, r: params.height - 1 } });
}
...
class ExcelBuilder {
  ...
  build(params) {
    var _a;
    const workbook = XLSXUtils__namespace.utils.book_new();
    ...
    sheetCacheManager.getSheets().forEach((sheetConfig, sheetIndex) => {
        ...
        chunk.tables.forEach((tableIndex) => {
          ...
          if (hasTitle) {
            ...
            tableConfig.columns.forEach((_, colIndex) => {
              const titleCellRef = XLSXUtils__namespace.utils.encode_cell({ c: COL_OFFSET + colIndex, r: ROW_OFFSET });
          ...
          tableConfig.columns.forEach((column, colIndex) => {
            const headerCellRef = XLSXUtils__namespace.utils.encode_cell({ c: colIndex + COL_OFFSET, r: ROW_OFFSET + (chunk.hasTitle ? 1 : 0) });
            ...
            tableConfig.content.forEach((row, rowIndex) => {
              ...
              values.forEach((value, valueIndex) => {
                const cellRef = XLSXUtils__namespace.utils.encode_cell({
          ...
          if (tableCache.getNbExtraRows() > 0) {
            tableConfig.content.forEach((row, rowIndex) => {
              ...
              const start = XLSXUtils__namespace.utils.encode_cell({ c: COL_OFFSET, r: rowStart });
              const end = XLSXUtils__namespace.utils.encode_cell({ c: COL_OFFSET + tableConfig.columns.length - 1, r: rowStart + (currentRowHeight - 1) });
      ...
      XLSXUtils__namespace.utils.book_append_sheet(workbook, worksheet, sheetConfig.sheet.sheet);
    });
...
exports.ExcelSchemaBuilder = ExcelSchemaBuilder;
